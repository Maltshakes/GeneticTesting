package net.maltshakes.genetesting.genes.format;
...
import net.maltshakes.genetesting.genes.datamodel.CustomBettaPolyScaleDefinition;
import net.maltshakes.genetesting.genes.datamodel.PolyGeneDefinition;
import net.maltshakes.genetesting.genes.datamodel.PolyScaleDefinition;
...
public abstract class GeneFormatting {
    ...
    public static class DisplayEntry {
        ...
        DisplayEntry(String label, List<String> mappings, GeneType type, int dataIndex, BiPredicate<Integer, Integer> condition) {
            this.label = label;
            this.mappings = mappings;
            this.type = type;
            this.dataIndex = dataIndex;
    ...
    /**
     * Registers a mapping for a specific gene pair using a single descriptive string.
     * Used for genes where only one label is needed to represent if the gene in present or not.
    ...
    /**
     * Registers a mapping for a specific gene pair at the given data index.
     * This creates a {@link DisplayEntry} which maps two individual alleles into visual labels.
     * The {@code dataIndex} determines which pair in the gene array is being targeted.
     * @param label     The display name of the gene.
    ...
    /**
     * Registers a mapping for a specific sex-linked pair using a single descriptive string.
     * Used for genes where only one label is needed to represent if the gene in present or not.
     * @param label     The display name of the gene.
     * @param value     The single label to display when the gene condition is met.
     * @param type      The {@link GeneType} (BINARY or BINARY_INVERTED).
     * @param dataIndex The index representing the specific pair of genes in the array.
     */
    public void addSexLinkedPairMapping(String label, String mapping, GeneType type, int dataIndex) {
        displayEntries.add(new DisplayEntry(label, List.of(mapping), type, dataIndex).setSexLinked(true));
    }

    /**
     * Registers a mapping for a specific sex-linked gene pair at the given data index.
     * This creates a {@link DisplayEntry} which maps two individual alleles into visual labels.
     * The {@code dataIndex} determines which pair in the gene array is being targeted.
     * @param label     The display name of the gene.
     * @param values    A list of descriptive strings used to map raw gene values.
     ...
     * @param dataIndex The index representing the specific pair of genes in the array.
     */
    public void addSexLinkedPairMapping(String label, List<String> mappings, GeneType type, int dataIndex) {
        displayEntries.add(new DisplayEntry(label, mappings, type, dataIndex).setSexLinked(true));
    }

    /**
     * Registers a polymorphic gene mapping for a range of alleles where each allele 
     * is expected to be a binary value (1 or 2).
     * 
     * <p>This is a convenience method that defaults the maximum allele value to 2. 
     * The sum of the range is used to calculate a percentage between the minimum 
     * possible sum (all 1s) and maximum possible sum (all 2s) to select a mapping string.
     *
    ...
    /**
     * Registers a polymorphic gene mapping for a range of alleles with a custom 
     * maximum value per allele (for example, 1 to 10).
     * 
     * <p>The total sum of the alleles in the range {@code [start, end]} is calculated. 
     * The result is then normalized based on the {@code maxVal} to determine the
     * appropriate index in the {@code mappings} list.
     *
     * @param label    The display name for this gene entry in the book.
     ...
     */
    public void addPolyRangeMapping(String label, List<String> mappings, int start, int end, int maxVal) {
     ...
     */
    public void addPolyScaleMapping(String label, List<String> mappings, int[] negIndices, int[] posIndices) {
        int totalGenes = (negIndices != null ? negIndices.length : 0) + (posIndices != null ? posIndices.length : 0);
        ...
        if (mappings.size() != totalGenes + 1) {
            throw new IllegalArgumentException(String.format(
                ...
                label, totalGenes + 1, mappings.size(), 
    ...
    /**
     * Registers a legacy gene that is hidden if both alleles are the specified wildtype value.
     * If either allele differs from the wildtype, the entry is displayed.
     * 
     ...
     */
    public void addLegacyMapping(String label, List<String> values, GeneType type, int dataIndex, int wildtypeValue) {
        displayEntries.add(new DisplayEntry(label, values, type, dataIndex, 
     ...
     */
    public void addCustomBettaPolyScaleMapping(String label, List<String> mappings, int[] firstPair, int[] secondPair) {
        // Range -4 to +2 = 7 possible labels
        if (mappings.size() != 7) {
            throw new IllegalArgumentException("Custom mapping for " + label + " requires exactly 7 labels.");
        }
     ...
     * @param condition A condition taking (val1, val2). Example: (v1, v2) -> v1 == 2 && v2 == 2
     */
    public void addConditionalComment(String text, int dataIndex, BiPredicate<Integer, Integer> condition) {
    ...
    /**
     * Adds a page break to the geneBook. This is handled internally as a different column,
     * not as a new page as Minecraft books understand it.
     */
     ...
     * and mapping that sum to a descriptive category string.
     * Calculates the relative position of the sum between the minimum possible sum (all 1s) and the maximum
     * possible sum (all 2s). This percentage is then used to select an index from the 
     * provided mapping list.
    ...
    /**
     * Processes a {@link PolyScaleDefinition} by calculating a weighted sum from a set of alleles 
     * and mapping the result to a display component.
     * 
     * <p>The logic evaluates specific indices in the {@code agenes} array:
     * <ul>
     *   <li>Indices in {@code poly.negIndices} subtract 1 from the total if the value is 2.</li>
     *   <li>Indices in {@code poly.posIndices} add 1 to the total if the value is 2.</li>
     * </ul>
     * 
     * <p>The final sum is normalized based on the minimum possible value (the negative of the 
     * number of negative indices) to determine the index for the {@code poly.mappings} list.
     *
    ...
    /**
     * Processes a custom genetic scale to determine a display component based on specific gene values.
     * <p>
     * The logic calculates a cumulative score from two pairs of gene indices:
     * <ul>
     *   <li><b>First Pair:</b> Increases the sum if the gene value is 2, decreases if it is 3.</li>
     *   <li><b>Second Pair:</b> Decreases the sum if the gene value is 2.</li>
     * </ul>
     * <p>
     * The resulting sum (ranging from -4 to 2 based on logic, normalized to 0 by adding 4) is used 
     ...
     * 
     * BINARY: Maps 1 to "+" (wildtype), 2 to a specific label.
     * BINARY_INVERTED: Maps 2 to "+" (wildtype), 1 to a specific label.
     * POLY: Maps the raw value directly to an index in the entry's mapping list.
     * COLOUR: Converts a 0-255 value into a coloured "█" symbol with a hex-code.
     * 
    ...
    private Component processGeneValue(DisplayEntry entry, int val) {
        ...
        if (type == GeneType.COLOUR) {
            ...
            return Component.literal("█").withStyle(style -> 
                style.withColor(TextColor.fromRgb(hueRGB))
                     // Tooltip: Shows the Hex code when hovered
                     .withHoverEvent(new HoverEvent(
                         HoverEvent.Action.SHOW_TEXT, 
                         Component.literal("Hex: ").append(Component.literal(hexCode)
                            .withStyle(Style.EMPTY.withColor(TextColor.fromRgb(hueRGB))))
    ...
    /**
     * Transforms raw gene data into a displayable list of entries based on pre-defined 
     * visibility conditions, labels, and specialized formatting rules.
     * 
     * This method handles page/line breaks, conditional visibility for "Other" gene types, 
     * polygenic gene calculations, and standard gene value mapping.
     * 
     * @param geneArray the raw integer array of gene values.
     * @return a list of {@link BookEntry} objects formatted for the UI.
     */
    public List<BookEntry> mapToEntries(int[] agenes, int[] sgenes, boolean isFemale) {
        ...
        for (DisplayEntry entry : displayEntries) {
            ...
            int v1Idx = entry.dataIndex * 2;
            int v2Idx = v1Idx + 1;
            Component val1 = (v1Idx < currentArray.length) ? processGeneValue(entry, currentArray[v1Idx]) : Component.literal("?");
            ...
            if (entry.isSexLinked && isFemale) {
                ...
                Component val2 = (v2Idx < currentArray.length) ? processGeneValue(entry, currentArray[v2Idx]) : Component.literal("?");

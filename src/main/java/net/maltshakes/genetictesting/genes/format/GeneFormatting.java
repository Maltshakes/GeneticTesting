...
public abstract class GeneFormatting {
    ...
    /**
     * Registers a mapping for a specific gene pair using a single descriptive string. Used for
    ...
    /**
     * Registers a mapping for a specific sex-linked pair using a single descriptive string. Used
     * for genes where only one label is needed to represent if the gene in present or not.
     *
     * @param label The display name of the gene.
     * @param value The single label to display when the gene condition is met.
     * @param type The {@link GeneType} (BINARY or BINARY_INVERTED).
    ...
    /**
     * Registers a mapping for a specific sex-linked gene pair at the given data index. This creates
     * a {@link DisplayEntry} which maps two individual alleles into visual labels. The {@code
    ...
    /**
     * Registers a polymorphic gene mapping for a range of alleles where each allele is expected to
     * be a binary value (1 or 2).
     *
     ...
     *
     * <p>The total sum of the alleles in the range {@code [start, end]} is calculated. The result
     * is then normalized based on the {@code maxVal} to determine the appropriate index in the
     * {@code mappings} list.
     *
     * @param label The display name for this gene entry in the book.
    ...
    /**
     * Processes a {@link PolyGeneDefinition} by summing the values of genes within a specified
     * range and mapping that sum to a descriptive category string. Calculates the relative position
     * of the sum between the minimum possible sum (all 1s) and the maximum possible sum (all 2s).
     * This percentage is then used to select an index from the provided mapping list.
     *
     ...
     * </ul>
     *
     * <p>The resulting sum (ranging from -4 to 2 based on logic, normalized to 0 by adding 4) is
     * used as an index to retrieve a string mapping, which is returned as a {@link Component}.
     *
     ...
     */
    private Component processCustomBettaPolyScale(
            CustomBettaPolyScaleDefinition poly, int[] agenes) {
        int totalSum = 0;
     ...
     * (wildtype), 1 to a specific label. POLY: Maps the raw value directly to an index in the
     * entry's mapping list. COLOUR: Converts a 0-255 value into a coloured "█" symbol with a
     * hex-code.
     *
                    ...
                    .withStyle(
                            style ->
                                    style.withColor(TextColor.fromRgb(hueRGB))
                                            ...
                                            // Click Action: Copies the hex code to the player's
                                            // clipboard
                                            .withClickEvent(
                                                    new ClickEvent(
     ...
     * structural elements (headers, breaks), and process polygenic calculations. For sex-linked
     * genes in females, it automatically collapses the display to a single allele.
     *

...
public class GeneBookDisplay {
    ...
    public static final int COLUMN_WIDTH = 110;

    /**
     * Constructs a display layout from a raw list of genetic entries. Logic includes: Formatting
     * headers with underline and comments with italics. Automatic column wrapping after 12 lines,
     * set to COLUMN_SIZE. Handling manual page and column breaks via metadata tags.
     *
    ...
    public GeneBookDisplay(List<BookEntry> entries) {
        ...
        for (BookEntry entry : entries) {
            // Page break logic
            if (entry.getType() == BookEntry.Type.COMMENT
                    && "[PAGE_BREAK]".equals(entry.getLabel().getString())) {
                if (!targetRight) {
                    targetRight = true;
                } else {
            ...
            MutableComponent line =
                    switch (entry.getType()) {
                        ...
                        case COMMENT -> {
                            if ("[CLEAN_BREAK]".equals(entry.getLabel().getString()))
            ...
            int spaceBeforeHeader =
                    (entry.getType() == BookEntry.Type.HEADER && currentLineCount > 0) ? 1 : 0;
            ...
            List<Component> targetCol =
                    targetRight ? currentPage.rightColumn : currentPage.leftColumn;
    ...
    /**
     * Reconstructs a display layout by parsing the NBT data of an existing ItemStack. This is
     * primarily used for reading books that have already been saved to the world.
     *
    ...
    public GeneBookDisplay(ItemStack stack) {
        ...
        if (tag == null || !tag.contains("pages", 9)) {
            ...
            warningPage.leftColumn.add(
                    Component.literal("USE A VANILLA MINECRAFT BOOK ON A GENETIC ANIMAL")
            ...
            for (int i = 0; i < pagesList.size(); i++) {
                try {
                    ...
                    List<FormattedCharSequence> lines =
                            Minecraft.getInstance().font.split(pageComponent, 2000);
    ...
    /**
     * Represents a single physical page in the GeneBook, partitioned into left and right columns.
     */
